...
const PROVIDERS: ProviderDef[] = [
  {
    ...
    fields: [
      { name: 'api_key', label: 'API Key', placeholder: 'sk_live_... or rk_live_...', required: true },
  ...
  },
  {
    ...
    label: 'AWS Bedrock',
    icon: '🧠',
    description: 'AI model inference via Amazon Bedrock. Skip if running on ECS with IAM role access.',
    ...
    fields: [
      ...
      { name: 'secret_access_key', label: 'Secret Access Key', type: 'password', placeholder: 'wJalr...', required: true },
  ...
  },
  {
    ...
    label: 'Office 365',
    icon: '📧',
    description: 'Microsoft Graph API for Outlook mail and calendar OAuth.',
    ...
    fields: [
      ...
      { name: 'client_secret', label: 'Client Secret', type: 'password', placeholder: 'xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx', required: true },
  ...
  },
  {
    ...
    icon: '📬',
    description: 'Gmail and Google Calendar access via Google OAuth.',
    docsUrl: 'https://console.cloud.google.com/apis/credentials',
    docsLabel: 'Google Cloud Console → Credentials',
    fields: [
      { name: 'client_id', label: 'Client ID', placeholder: 'xxxx.apps.googleusercontent.com', required: true },
  ...
  },
  {
    ...
    fields: [
      ...
      { name: 'environment', label: 'Environment', placeholder: 'sandbox | development | production' },
    ],
  },
  {
    id: 'anthropic',
    ...
    fields: [
      { name: 'api_key', label: 'API Key', type: 'password', placeholder: 'sk-ant-api03-...', required: true },
    ],
  },
  {
    id: 'openai',
  ...
  },
  {
    ...
    fields: [
      { name: 'api_key', label: 'API Key', type: 'password', placeholder: 'SG....', required: true },
...
export default async function AdminIntegrationsPage() {
  ...
  const configuredProviders = new Set(credentials.filter((c) => c.enabled).map((c) => c.provider));
    ...
    <div className="flex flex-col gap-6">
      <div>
        ...
        <p className="text-text-secondary m-0 mt-2 text-[0.9rem]">
          Connect external services. All credentials are encrypted at rest (AES-256-GCM).
        </p>
      </div>
                ...
                <div className="border-border border-t px-5 py-3">
                  <div className="flex flex-wrap items-center gap-3 text-[0.8rem]">
                    ...
                    {existing.lastTestResult ? (
                      <span className={`rounded-full px-2 py-0.5 text-[0.72rem] font-extrabold ${testResultColor[existing.lastTestResult] ?? 'bg-wash text-text-secondary'}`}>
                        {existing.lastTestResult}
                      </span>
                    ) : null}
                  ...
                  <div className="mt-2.5 flex gap-2">
                    <form action={handleTest}>
                      ...
                      <Button type="submit" variant="quiet" size="small">Test</Button>
                    </form>
                    ...
                    <form action={handleDelete}>
                      <input type="hidden" name="id" value={existing.id} />
                      <Button type="submit" variant="ghost" size="small">Remove</Button>
                    </form>
                  </div>
                </div>
              ) : (
                ...
                <form
                  action={async (formData: FormData) => {
                    ...
                    await createIntegrationCredential({
                      provider: formData.get('provider') as 'stripe' | 'aws_bedrock' | 'office365' | 'google_workspace' | 'plaid' | 'openai' | 'anthropic' | 'sendgrid' | 'twilio' | 'custom',
            ...
            <table className="w-full border-collapse text-[0.82rem]">
              <thead>
                <tr className="bg-wash text-left">
                  <th className="px-4 py-2 font-extrabold text-ink">Provider</th>
                  <th className="px-4 py-2 font-extrabold text-ink">Label</th>
                  <th className="px-4 py-2 font-extrabold text-ink">Status</th>
                  <th className="px-4 py-2 font-extrabold text-ink">Last Test</th>
                  <th className="px-4 py-2 font-extrabold text-ink">Created</th>
                </tr>
              </thead>
              <tbody className="divide-border divide-y">
                {credentials.map((cred) => (
                  <tr key={cred.id}>
                    ...
                    <td className="px-4 py-2 text-text-secondary">{cred.label}</td>
                    <td className="px-4 py-2">
                      <span className={`rounded-full px-2 py-0.5 text-[0.72rem] font-extrabold ${cred.enabled ? 'bg-wash-green text-signal' : 'bg-risk-wash text-ink'}`}>
                        {cred.enabled ? 'active' : 'disabled'}
                      </span>
                    </td>
                    <td className="px-4 py-2">
                      {cred.lastTestResult ? (
                        <span className={`rounded-full px-2 py-0.5 text-[0.72rem] font-extrabold ${testResultColor[cred.lastTestResult] ?? 'bg-wash text-text-secondary'}`}>
                          {cred.lastTestResult}
                        </span>
                      ) : '—'}
                    </td>
