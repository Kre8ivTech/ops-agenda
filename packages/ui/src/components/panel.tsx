...
export function PanelHeading({ children }: { children: ReactNode }) {
  return (
    <div className="border-border bg-wash text-text-secondary border-b px-6 py-[14px] font-mono text-[0.72rem] font-extrabold uppercase leading-none tracking-[0.1em]">
      {children}
    </div>
  );
}
...
export function InkGridCell({
  ...
  return (
    <div className={`bg-ink min-w-0 grow px-6 py-[22px] ${className}`} style={{ flexBasis: basis }}>
      <p
        className={
          isEyebrow
            ? 'text-signal-on-ink m-0 mb-[7px] font-mono text-[0.72rem] font-bold uppercase leading-none tracking-[0.08em]'
