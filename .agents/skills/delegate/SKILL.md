---
name: delegate
description: Delegate tasks to specialized subagents. Analyzes requirements, selects appropriate agents, and launches them with proper context while enforcing the max-2-parallel rule.
---
...
1. Read `.Codex/config/coding-standards.json` for detection rules
...
Read `Codex-activeContext.md` to check parallel execution tracking:
...
If current phase is known (from `Codex-activeContext.md`), prefer phase-appropriate agent:
...
Task:
  ...
  prompt: |
    ## Context
    [Current phase and project context from Codex-activeContext.md]
    ...
    ## Constraints
    - Follow patterns in Codex-patterns.md
    - Follow coding standards loaded above
    - Update Codex-activeContext.md when complete
...
Map agents to Codex subagent types:
...
1. Update `Codex-activeContext.md`:
...
- **Memory bank missing**: Create `Codex-activeContext.md` with default structure
